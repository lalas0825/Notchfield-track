...
export type NodRenderInput = {
  ...
  trackingToken: string;    // embedded in tracking pixel URL
  signedAtIso: string;
...
function buildHtml(input: NodRenderInput, trackingPixelUrl: string): string {
...
<body>
  <div class="page">
    ...
    <!-- Tracking pixel (loaded by the email client when the GC opens the message; the PDF renderer ignores it) -->
    <img src="${esc(trackingPixelUrl)}" width="1" height="1" alt=""/>
  </div>
</body>
</html>`;
}
...
export async function renderAndUploadNod(
  ...
  const html = buildHtml(input, trackingPixelUrl);
