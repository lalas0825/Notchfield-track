...
| 53A | **Communication v1 + Push Notifications** (2026-04-24/25). Per-area threads + push fanout via Edge Function. New `device_tokens` table + Expo push pipeline. `field_messages` UI wired to AreaDetail with `MessageThread` + `MessageComposer` + photo support. `blockPhase` migrated to `createSystemMessage` with `[SYS:blocker]` prefix → 🔒 icon. Activity badges on Ready Board area cards. `fanout-field-message` Edge Function deployed (status ACTIVE) + Database Webhook on field_messages INSERT. End-to-end push validated in field test. Realtime cross-app channel via Supabase. Full spec in [SPRINT_TRACK_53.md](SPRINT_TRACK_53.md) + [SPRINT_53_TAKEOFF_COORDINATION.md](SPRINT_53_TAKEOFF_COORDINATION.md). | ✅ |
| 53B | **Punch List polish + plan pinning** (2026-04-25). `AddPunchSheet` with title/priority/area-picker/photo upload. `PunchPinOverlay` for Plans tab; FAB-based drop-pin flow. `useAssignableProfiles` hook (FK = profiles.id, not workers.id). `useDrawingPunchItems` + `useAreaPunchActivity`. Photo upload via `uploadPunchPhoto` (RLS-correct path). Bug fix: `isSupervisor` check now uses `normalizeTrackRole` (3 places — Sprint 40C role canonicalization had been silently locking out users with role='supervisor'). | ✅ |
| 53C | **Legal Engine fix + complete v1** (2026-04-25). Critical bug fix: `LegalDoc.status` enum aligned with DB CHECK (`draft → sent → opened → no_response`, no 'signed' state). Sign+send is one transaction. Track-side `nodPdfRenderer.ts` (local expo-print, OrgLetterhead, NY DOB boilerplate). `costEngine.ts` reads `area_time_entries` + `workers.daily_rate_cents` + writes to `delay_cost_logs`. **Option B chosen 2026-04-25**: Track calls Web's `/api/pm/legal-documents/[docId]/distribute` (deployed by Web team commit `cc16f75`) instead of duplicating Zoho creds in Track Edge Function. `NodSignModal` + `DelayCostCard`. Web also ships `/api/legal/{token}/pixel` for open-receipt tracking. | ✅ |
| 53A.1 | **Field-test fixes** (2026-04-25). Round of fixes after Jantile started testing the rebuilt APK: (1) **General channel UI** — new `/messages` tab between Delivery and More with `chatbubbles` icon, `tabBarBadge` with auto-reset on visit (DeviceEventEmitter pattern); push deep-link routes to /messages when area_id IS NULL. (2) **Storage RLS path order** — `field-photos` bucket policy requires `{org_id}/...` as first folder, my MessageComposer + uploadPunchPhoto + nodPdfRenderer + signature upload all had it reversed (`messages/{org_id}/...` etc.) → 403 silent on every upload. Fixed all 4. (3) **`messages/_layout.tsx` missing** — every tab folder in the project has its own _layout.tsx; without it `<Tabs.Screen name="messages">` config silently bypasses → tab appeared as auto-discovered placeholder at end with broken icon ("messa..." truncated). No cache clear / force-quit / EAS rebuild fixes this — only adding the missing file does. (4) **Auto sign-out when refresh_token dies** — `fetchCredentials` now refreshes if expiring within 60s; if refresh fails (refresh_token expired), signs out cleanly so AuthGate redirects to /login. AppState 'active' listener does the same on foreground. (5) **SyncStatusBar root cause** — was reading `status.dataFlowStatus` (doesn't exist; correct is `dataFlow`) AND subscribing via `statusUpdates.subscribe()` (also doesn't exist; correct is `registerListener({ statusChanged })`). Banner stuck on initial state forever, falsely showing "Reconnecting…" while PowerSync was actually fine. Discovered via new Sync Diagnostics screen (More → Sync Diagnostics, hidden by default after fix landed). (6) **Offline vs Reconnecting distinction** — 5s threshold differentiates brief drops ("Reconnecting…") from sustained outages ("Offline — changes saved locally"). (7) **Photo bubble height + Notes-last + internal scroll** — horizontal photo ScrollView needed explicit height: 96; Notes section moved to bottom of AreaDetail; messages container has internal vertical ScrollView with maxHeight: 360 and `nestedScrollEnabled`. (8) **VirtualizedList nesting warning** removed from MessageThread (FlatList → View.map; cap at 50 messages so virtualization not needed). (9) **Photo offline dialog** copy clarified: "You're offline. Send the text now and add the photos later in a new message when you're back online?". (10) **Keyboard covered input on Android** — KAV at screen-level (ProjectNotesScreen + AreaDetail) with `behavior=height` for Android, `behavior=padding` for iOS; also added `expo.android.softwareKeyboardLayoutMode: "resize"` to app.json (kicks in on next EAS build). (11) **`react-native-blob-util` + `react-native-webview`** added as explicit peer deps after `npm install --legacy-peer-deps` for expo-notifications/expo-device dropped them from the lockfile. Added `legacy-peer-deps=true` `.npmrc`. (12) Tab label "Messages" shortened to "Notes" to fit. | ✅ |
